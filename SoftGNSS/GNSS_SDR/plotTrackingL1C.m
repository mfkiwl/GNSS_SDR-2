function plotTrackingL1C(channelList, trackResults, settings)
...
%% Select (or create) and clear the figure ================================
    ...
    set(channelNr +200, 'Name', ['Channel ', num2str(channelNr), ...
                                 ...
                                 ' ' ...
                                 trackResults(channelNr).codeSelection ...
                                 ') results']);
...
%% Plot all figures =======================================================
        ...
        timeAxisInSeconds = (10:10:settings.msToProcess)/1000;
        ...
        ylabel(handles(4, 1), 'Hz');
        title ('Parent',handles(4, 1), ['Carrier Frequency'],...
            'Units','normalized');
        ...
        ylabel(handles(4, 2), 'Hz');
        title ('Parent',handles(4, 2), ['Code Frequency'],...
            'Units','normalized');
        
        % ---- C/No--------------------------------------------------------
        plot(handles(4, 3),...
             trackResults(channelNr).CNo.VSMIndex/100, trackResults(channelNr).CNo.VSMValue,'b');
        grid (handles(4, 3));
        %axis (, 'tight');
        axis (handles(4, 3), [0.01 settings.msToProcess/1000 -inf inf]);
        xlabel(handles(4, 3),'Time (s)');
        ylabel(handles(4, 3),'dB-Hz');
        title ('Parent',handles(4, 3), 'C/No','Units','normalized');
...
%% Select (or create) and clear the figure ================================
    ...
    % this function.
        ...
        set(channelNr +300, 'Name', ['Channel ', num2str(channelNr), ...
                                     ' (PRN ', ...
                                     num2str(trackResults(channelNr + settings.numberOfChannels).PRN), ...
                                     ' ' ...
                                     trackResults(channelNr + settings.numberOfChannels).codeSelection, ...
                                     ') results']);
...
%% Plot all figures =======================================================
        ...
        %----- Discrete-Time Scatter Plot ---------------------------------
        plot(handles(1, 1), trackResults(channelNr + settings.numberOfChannels).I_P,...
                            trackResults(channelNr + settings.numberOfChannels).Q_P, ...
                            '.');
        ...
        %----- Nav bits ---------------------------------------------------
        plot  (handles(1, 2), timeAxisInSeconds, ...
                              trackResults(channelNr + settings.numberOfChannels).I_P);
        ...
        %----- PLL discriminator unfiltered--------------------------------
        plot  (handles(2, 1), timeAxisInSeconds, ...
                              trackResults(channelNr + settings.numberOfChannels).pllDiscr, 'r');      
        ...
        %----- Correlation ------------------------------------------------
        plot(handles(2, 2), timeAxisInSeconds, ...
                            [sqrt(trackResults(channelNr + settings.numberOfChannels).I_E.^2 + ...
                                  trackResults(channelNr + settings.numberOfChannels).Q_E.^2)', ...
                             sqrt(trackResults(channelNr + settings.numberOfChannels).I_P.^2 + ...
                                  trackResults(channelNr + settings.numberOfChannels).Q_P.^2)', ...
                             sqrt(trackResults(channelNr + settings.numberOfChannels).I_L.^2 + ...
                                  trackResults(channelNr + settings.numberOfChannels).Q_L.^2)'], ...
                            '-*');
        ...
        %----- PLL discriminator filtered----------------------------------
        plot  (handles(3, 1), timeAxisInSeconds, ...
                              trackResults(channelNr + settings.numberOfChannels).pllDiscrFilt, 'b');      
        ...
        %----- DLL discriminator unfiltered--------------------------------
        plot  (handles(3, 2), timeAxisInSeconds, ...
                              trackResults(channelNr + settings.numberOfChannels).dllDiscr, 'r');      
        ...
        %----- DLL discriminator filtered----------------------------------
        plot  (handles(3, 3), timeAxisInSeconds, ...
                              trackResults(channelNr + settings.numberOfChannels).dllDiscrFilt, 'b');      
        ...
        %----- Carrier Frequency----------------------------------
        plot  (handles(4, 1), timeAxisInSeconds, ...
            trackResults(channelNr + settings.numberOfChannels).carrFreq, 'Color',[0.42 0.25 0.39]);
        ...
        %----- Code Frequency----------------------------------
        plot  (handles(4, 2), timeAxisInSeconds, ...
            trackResults(channelNr + settings.numberOfChannels).codeFreq, 'Color',[0.2 0.3 0.49]);
        grid  (handles(4, 2));
        axis  (handles(4, 2), 'tight');
        xlabel(handles(4, 2), 'Time (s)');
        ylabel(handles(4, 2), 'Hz');
        ...
        % ---- C/No--------------------------------------------------------
        plot(handles(4, 3),...
            trackResults(channelNr + settings.numberOfChannels).CNo.VSMIndex/100, trackResults(channelNr + settings.numberOfChannels).CNo.VSMValue,'b');
        grid (handles(4, 3));
