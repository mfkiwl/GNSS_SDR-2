function L1CDCodesTable = makeL1CDTable(settings)
...
%One row in the "L1CDCodesTable" is one L1CD code. The row number is the PRN
%number of the L1CD code.
%
...
%   Outputs:
%       L1CDCodesTable  - an array of arrays (matrix) containing L1CD codes
...
L1CDCodesTable = zeros(32, samplesPerCode);
bocCodes = [1 -1]; 
%--- Find time constants --------------------------------------------------
...
for PRN = 1:32
    ...
    L1CDCodesTable(PRN, :) = L1CDCode(codeValueIndex) .* bocCodes(bocCodeValueIndex);
