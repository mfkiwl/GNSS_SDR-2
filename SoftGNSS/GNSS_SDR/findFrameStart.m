function [firstSubFrame, activeChnList] = findFrameStart(trackResults, ...
    settings)
% findFrameStart finds the first subframe occurrence in the bit stream of
...
%
%[firstSubFrame, activeChnList] = findFrameStart(trackResults, settings)
%
%   Inputs:
...
for channelNr = activeChnList
    ...
    if trackResults(channelNr).codeSelection == "L1CP"
        %--- Generate the preamble pattern ----------------------------------------
        L1COcode = generateL1COcode(trackResults(channelNr).PRN);
        ...
        bits = trackResults(channelNr).I_P(1 + searchStartOffset : searchEnd);
        ...
        tlmXcorrResult = xcorr(bits, L1COcode);
...
%         plot(tlmXcorrResult);
        ...
        %--- Find at what index/ms the frame start ------------------------
        index = find(...
            abs(tlmXcorrResult(xcorrLength : xcorrLength * 2 - 1)) > 1500)' + ...
            searchStartOffset;
        
        if ~isempty(index)
            
            firstSubFrame(channelNr) = max(index);

            %--- Apple frame start from L1CP to L1CD ------------------------
            for channeL1CD = activeChnList
                if(trackResults(channeL1CD).PRN == trackResults(channelNr).PRN) & ...
                    (trackResults(channeL1CD).codeSelection == "L1CD")
                    firstSubFrame(channeL1CD) = firstSubFrame(channelNr);
                end
            end
        end
    end
